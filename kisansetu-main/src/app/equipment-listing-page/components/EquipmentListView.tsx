    ...
    <>
      <div className="space-y-3">
        {equipment.map((item) => (
          <div
            key={item.id}
            className="card-base overflow-hidden hover:border-primary/30 hover:shadow-card-hover transition-all duration-200 group"
          >
                ...
                <button
                  ...
                  className={`absolute top-2 right-2 w-7 h-7 rounded-full flex items-center justify-center transition-all ${
                    wishlist.includes(item.id)
                      ? 'bg-danger text-white' :'bg-white/80 text-muted-foreground hover:text-danger'
                  }`}
                >
              ...
              <div className="flex-1 p-4 flex flex-col justify-between min-w-0">
                <div className="flex items-start justify-between gap-2">
                  <div className="min-w-0">
                    ...
                    <div className="flex flex-wrap items-center gap-3 text-xs">
                      <div className="flex items-center gap-1">
                        ...
                        <span className="font-bold font-tabular">{item.rating}</span>
                      ...
                      <span className={`px-2 py-0.5 rounded-full text-xs font-semibold ${
                        item.available ? 'bg-success-bg text-success' : 'bg-danger-bg text-danger'
                      }`}>
                        {item.available ? `Available (${item.units} units)` : 'Booked'}
                      </span>
                    </div>
                  </div>
                  ...
                  <div className="text-right shrink-0 hidden sm:block">
                    ...
                    {item.listingType !== 'rent' && (
                      <p className="text-xs text-muted-foreground font-tabular">Buy: ₹{(item.buyPrice / 100000).toFixed(1)}L</p>
                    )}
                ...
                <div className="flex items-center gap-3 mt-2 sm:hidden">
                  <div className="flex items-center gap-1 text-xs text-muted-foreground">
                    <Tag size={10} />
                    <span className="font-bold text-primary font-tabular">₹{item.rentPerDay}/day</span>
                  </div>
                ...
                <div className="flex items-center gap-2 mt-3 pt-3 border-t border-border">
                  ...
                  <button
                    ...
                    className={`px-4 py-1.5 rounded-lg text-xs font-bold transition-all btn-press ${
                      item.available
                  ...
                  {item.listingType !== 'rent' && (
                    <button className="btn-accent text-xs py-1.5 px-4">
                      Buy Now
                    </button>
                  )}
