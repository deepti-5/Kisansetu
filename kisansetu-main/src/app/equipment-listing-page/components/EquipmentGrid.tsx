...
export default function EquipmentGrid({ equipment }: Props) {
  ...
  function toggleWish(id: string, name: string) {
    setWishlist((prev) => {
      ...
      toast.success(`${name} saved to wishlist ❤️`);
      return [...prev, id];
    });
  }
            ...
            <div className="relative h-48 overflow-hidden shrink-0">
              ...
              <button
                ...
                className={`absolute top-3 right-3 w-8 h-8 rounded-full flex items-center justify-center transition-all duration-150 ${
                  wishlist.includes(item.id)
                    ? 'bg-danger text-white scale-110' :'bg-white/80 backdrop-blur-sm text-muted-foreground hover:bg-white hover:text-danger'
                }`}
              >
              ...
              <div className="absolute bottom-3 left-3">
                <span
                  className={`px-2.5 py-1 rounded-full text-xs font-bold ${
                    item.available ? 'bg-success text-white' : 'bg-danger/90 text-white'
                  }`}
                >
              ...
              <div className="absolute bottom-3 right-3">
                <span className={`px-2 py-0.5 rounded-full text-xs font-semibold ${
                  item.listingType === 'rent' ? 'bg-blue-500 text-white' :
                  item.listingType === 'buy'? 'bg-accent text-white' : 'bg-primary text-white'
                }`}>
            ...
            <div className="p-4 flex flex-col flex-1">
              ...
              <div className="bg-muted/60 rounded-lg p-3 mb-3 space-y-1.5">
                ...
                <div className="flex items-center justify-between pt-1 border-t border-border">
                  ...
                  <span className="text-xs font-bold text-warning font-tabular">
                    ₹{item.deposit.toLocaleString('en-IN')} (refundable)
                  </span>
                </div>
              </div>
              ...
              <div className="mt-auto grid grid-cols-3 gap-2">
                <button
                  ...
                  className={`col-span-1 flex items-center justify-center gap-1 py-2 rounded-lg text-xs font-bold transition-all btn-press ${
                    item.available
                      ? 'gradient-green text-white hover:opacity-90' :'bg-muted text-muted-foreground cursor-not-allowed'
                  }`}
                >
                  Rent
                </button>
                {item.listingType !== 'rent' ? (
                  <button className="col-span-1 btn-accent text-xs py-2 justify-center">
                    Buy
                  </button>
                ) : (
                  <button className="col-span-1 flex items-center justify-center gap-1 py-2 rounded-lg border border-border text-xs font-semibold text-muted-foreground hover:bg-muted transition-colors">
                    <Phone size={12} /> Call
                  </button>
                )}
                <Link href={`/equipment/${item.id}`} className="col-span-1 flex items-center justify-center gap-1 py-2 rounded-lg border border-border text-xs font-medium text-muted-foreground hover:bg-muted transition-colors">
                  <Eye size={12} /> View
                </Link>
              </div>
            </div>
