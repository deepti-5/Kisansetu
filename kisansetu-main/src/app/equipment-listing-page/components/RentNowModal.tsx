...
import {
  X, Calendar, Clock, ChevronRight, CheckCircle,
...
export default function RentNowModal({ equipment, onClose }: Props) {
  ...
  function handlePaymentSuccess(result: PaymentResult) {
    ...
    setPaidPaymentId(result.paymentId);
    ...
    <div className="fixed inset-0 z-[80] flex items-end sm:items-center justify-center p-0 sm:p-4 bg-black/50">
      <div className="bg-card w-full sm:max-w-lg rounded-t-3xl sm:rounded-2xl shadow-modal max-h-[95vh] overflow-y-auto fade-in">
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b border-border sticky top-0 bg-card z-10">
          <div>
            <h3 className="font-bold text-base text-foreground">Rent Equipment</h3>
            <p className="text-xs text-muted-foreground line-clamp-1">{equipment.name}</p>
          </div>
            ...
            {STEPS.map((s, i) => (
              <React.Fragment key={`step-ind-${s.key}`}>
                <div className="flex flex-col items-center gap-1">
                  ...
                  <span className={`text-xs font-medium ${step === s.key ? 'text-primary' : 'text-muted-foreground'}`}>
                    {s.label}
                  </span>
                </div>
                {i < STEPS.length - 1 && (
                  <div className={`flex-1 h-0.5 mx-2 transition-colors ${
                    STEPS.findIndex((x) => x.key === step) > i ? 'bg-success' : 'bg-border'
                  }`} />
                )}
              </React.Fragment>
            ))}
          ...
          {step === 'dates' && (
            <div className="space-y-4 slide-up">
              <div className="flex items-center gap-3 p-3 bg-muted/40 rounded-xl">
                ...
                <div>
                  ...
                  <div className="flex items-center gap-2 text-xs text-muted-foreground">
                    ...
                    {equipment.rating} · <MapPin size={10} className="text-primary" /> {equipment.distance} km
                  </div>
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-sm font-semibold text-foreground mb-1.5">
                    <Calendar size={13} className="inline mr-1.5 text-primary" />
                    Start Date
                  </label>
                  <input
                    type="date"
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-sm font-semibold text-foreground mb-1.5">
                    <Clock size={13} className="inline mr-1.5 text-primary" />
                    Pickup Time
                  </label>
                  <input
                    type="time"
              ...
              {startDate && endDate && (
                <div className="bg-secondary/60 rounded-xl p-3 text-sm">
                  ...
                  <div className="flex items-center justify-between mt-1">
                    ...
                    <span className="font-bold text-foreground font-tabular">₹{rentalAmount.toLocaleString('en-IN')}</span>
                  </div>
                </div>
              )}
            ...
            <div className="space-y-4 slide-up">
              ...
              <div className="space-y-2.5 text-sm">
                {[
                  ...
                  { label: 'Start', value: `${startDate} · ${startTime}` },
                  { label: 'End', value: `${endDate} · ${endTime}` },
                  { label: 'Duration', value: `${days} day${days > 1 ? 's' : ''}` },
                ].map((row) => (
                  <div key={`summary-${row.label}`} className="flex justify-between">
                    <span className="text-muted-foreground">{row.label}</span>
                    <span className="font-semibold text-foreground text-right max-w-[200px] font-tabular">{row.value}</span>
                  </div>
                ))}

                <div className="border-t border-border pt-2.5 space-y-2">
                  <div className="flex justify-between">
                    <span className="text-muted-foreground">Rental ({days}d × ₹{equipment.rentPerDay.toLocaleString('en-IN')})</span>
                    <span className="font-tabular">₹{rentalAmount.toLocaleString('en-IN')}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-muted-foreground">Security Deposit</span>
                    <span className="text-warning font-semibold font-tabular">₹{equipment.deposit.toLocaleString('en-IN')}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-muted-foreground">Platform Fee (2%)</span>
                    <span className="font-tabular">₹{platformFee.toLocaleString('en-IN')}</span>
                  </div>
                </div>

                <div className="border-t border-border pt-2.5 flex justify-between">
                  <span className="font-bold text-foreground text-base">Total Payable</span>
                  <span className="font-bold text-xl text-primary font-tabular">₹{totalPayable.toLocaleString('en-IN')}</span>
                </div>
              </div>
            ...
            <div className="space-y-4 slide-up">
              ...
              <div className="bg-muted/40 rounded-xl p-3 text-xs space-y-1.5">
                ...
                <div className="flex justify-between text-muted-foreground">
                  ...
                  <span className="font-tabular text-warning">₹{equipment.deposit.toLocaleString('en-IN')}</span>
                </div>
                <div className="flex justify-between text-muted-foreground">
                ...
                <div className="flex justify-between font-bold text-foreground border-t border-border pt-1.5">
                  <span>Total</span>
                  <span className="font-tabular text-primary">₹{totalPayable.toLocaleString('en-IN')}</span>
                </div>
              </div>
              ...
              <div className="grid grid-cols-3 gap-2 text-center text-xs text-muted-foreground">
                {[
                  ...
                  <div key={m.label} className="bg-muted/40 rounded-lg py-2 px-1">
              ...
              <RazorpayCheckout
                amount={totalPayable}
                receipt={`kisan_${equipment.id}_${Date.now()}`}
                notes={{
                ...
                description={`Rental: ${equipment.name} (${days} day${days > 1 ? 's' : ''})`}
                buttonText={`Pay ₹${totalPayable.toLocaleString('en-IN')} via Razorpay`}
              ...
              <div className="flex items-center gap-2 text-xs text-muted-foreground bg-muted/40 rounded-lg p-2.5">
                ...
                100% secure payments powered by Razorpay. Supports UPI, Cards &amp; Net Banking.
              </div>
            </div>
          )}
          ...
          {step === 'confirmation' && (
            <div className="text-center py-4 slide-up">
              ...
              <p className="text-sm text-muted-foreground mb-4">
                Your equipment rental has been successfully booked and payment verified.
              </p>

              <div className="bg-muted/40 rounded-xl p-4 text-left space-y-2 mb-4 text-sm">
                <div className="flex justify-between">
                  <span className="text-muted-foreground">Booking ID</span>
                  <span className="font-bold text-primary font-tabular">{bookingId}</span>
                </div>
                {paidPaymentId && (
                  <div className="flex justify-between">
                    <span className="text-muted-foreground">Payment ID</span>
                    <span className="font-mono text-xs text-foreground truncate max-w-[160px]">{paidPaymentId}</span>
                  </div>
                )}
                <div className="flex justify-between">
                ...
                <div className="flex justify-between">
                  ...
                  <span className="font-bold text-foreground font-tabular">₹{totalPayable.toLocaleString('en-IN')}</span>
                </div>
                <div className="flex justify-between">
              ...
              <div className="bg-warning-bg border border-warning/20 rounded-lg p-3 text-left mb-4">
                ...
                <p className="text-xs text-muted-foreground">
                  Share this OTP with the equipment owner at the time of pickup. A return OTP will be generated when you initiate return.
                </p>
              </div>
        ...
        {/* Footer CTA — only for dates and summary steps */}
        ...
        {step === 'payment' && (
          <div className="px-5 pb-5 pt-2 border-t border-border sticky bottom-0 bg-card">
            <button
              ...
              className="btn-secondary py-3 px-5 w-full"
            >
              ← Back to Summary
            </button>
          </div>
