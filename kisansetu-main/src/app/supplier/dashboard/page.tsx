...
import { LayoutDashboard, Package, Calendar, IndianRupee, Plus, Star, TrendingUp, CheckCircle, Clock, XCircle, Eye, Edit, Trash2, Bell, ChevronRight, BarChart2, ArrowUpRight, Filter } from 'lucide-react';

interface Listing {
  id: string;name: string;category: string;image: string;
  rentPerDay: number;buyPrice: number;status: 'active' | 'paused' | 'pending';
  bookings: number;rating: number;earnings: number;
}

interface BookingReq {
  id: string;equipment: string;farmer: string;phone: string;
  startDate: string;endDate: string;amount: number;status: 'pending' | 'accepted' | 'declined';
  type: 'rent' | 'buy';
}

const LISTINGS: Listing[] = [
{ id: 'l1', name: 'Mahindra Yuvo 575 DI Tractor', category: 'Tractor', image: "https://img.rocket.new/generatedImages/rocket_gen_img_1315e840e-1765250553872.png", rentPerDay: 2500, buyPrice: 750000, status: 'active', bookings: 34, rating: 4.6, earnings: 85000 },
{ id: 'l2', name: 'Rotavator 7 Feet', category: 'Tillage', image: 'https://images.unsplash.com/photo-1574943320219-553eb213f72d?w=400', rentPerDay: 1200, buyPrice: 95000, status: 'active', bookings: 18, rating: 4.4, earnings: 21600 },
{ id: 'l3', name: 'Paddy Transplanter 8-Row', category: 'Planting', image: 'https://images.unsplash.com/photo-1560493676-04071c5f467b?w=400', rentPerDay: 3500, buyPrice: 0, status: 'paused', bookings: 9, rating: 4.2, earnings: 31500 },
{ id: 'l4', name: 'Combine Harvester', category: 'Harvesting', image: 'https://images.unsplash.com/photo-1625246333195-78d9c38ad449?w=400', rentPerDay: 8000, buyPrice: 2200000, status: 'pending', bookings: 0, rating: 0, earnings: 0 }];
...
{ id: 'BKG12338', equipment: 'Paddy Transplanter 8-Row', farmer: 'Anita Sharma', phone: '+91 65432 44444', startDate: '2026-08-05', endDate: '2026-08-07', amount: 10500, status: 'declined', type: 'rent' },
{ id: 'ORD00123', equipment: 'Rotavator 7 Feet', farmer: 'Ramesh Patil', phone: '+91 54321 55555', startDate: '2026-08-01', endDate: '2026-08-01', amount: 95000, status: 'accepted', type: 'buy' }];


const EARNINGS_MONTHLY = [
{ month: 'Mar', amount: 18000 }, { month: 'Apr', amount: 24000 }, { month: 'May', amount: 31000 },
{ month: 'Jun', amount: 28000 }, { month: 'Jul', amount: 42000 }, { month: 'Aug', amount: 15000 }];
...
export default function SupplierDashboard() {
  ...
  const totalBookings = bookings.filter((b) => b.status === 'accepted').length;

  function updateBooking(id: string, status: 'accepted' | 'declined') {
    ...
    <div className="min-h-screen bg-background">
      ...
      <main className="max-w-screen-xl mx-auto px-4 lg:px-8 py-6">
        ...
        <div className="flex items-center justify-between mb-6">
          <div>
            <h1 className="text-2xl font-extrabold text-foreground">Supplier Dashboard</h1>
        ...
        <div className="space-y-6">
            ...
            { label: 'Total Bookings', value: totalBookings, icon: Calendar, color: 'text-accent', bg: 'bg-accent/10', change: '+5' },
          ...
          <div className="bg-warning-bg border border-warning/30 rounded-2xl p-4">
              ...
              <div key={b.id} className="bg-card rounded-xl p-3 flex items-center justify-between gap-3 flex-wrap">
                      ...
                      <div className="flex gap-2">
                        <button onClick={() => updateBooking(b.id, 'accepted')} className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-success text-white text-xs font-bold hover:bg-success/90 transition-colors">
                          <CheckCircle size={13} /> Accept
                        </button>
                        <button onClick={() => updateBooking(b.id, 'declined')} className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-danger text-white text-xs font-bold hover:bg-danger/90 transition-colors">
                  ...
                  <div className="p-4">
                    ...
                    <div className="grid grid-cols-3 gap-2 text-center mb-3">
                      <div className="bg-muted/40 rounded-lg py-2">
                        ...
                        <p className="font-bold text-sm text-foreground">{l.bookings}</p>
                      </div>
                      <div className="bg-muted/40 rounded-lg py-2">
                        <p className="text-xs text-muted-foreground">Rating</p>
                        <p className="font-bold text-sm text-foreground">{l.rating > 0 ? `${l.rating}★` : '—'}</p>
                      </div>
                      <div className="bg-muted/40 rounded-lg py-2">
                        <p className="text-xs text-muted-foreground">Earned</p>
                        <p className="font-bold text-sm text-success">₹{(l.earnings / 1000).toFixed(0)}k</p>
                      </div>
                    </div>
                    <div className="flex gap-2">
                      ...
                      <button className="flex items-center justify-center gap-1.5 py-2 px-3 rounded-xl border border-danger/30 text-danger text-xs font-semibold hover:bg-danger/10 transition-colors"><Trash2 size={13} /></button>
                    </div>
                  </div>
                </div>
            )}
                  ...
                  <div key={b.id} className="bg-card rounded-2xl border border-border p-4">
                        <div className="flex items-start justify-between gap-3 flex-wrap">
                          <div className="space-y-1">
                            <div className="flex items-center gap-2">
                              <span className={`px-2 py-0.5 rounded-full text-xs font-bold ${b.type === 'rent' ? 'bg-primary/10 text-primary' : 'bg-accent/10 text-accent'}`}>{b.type === 'rent' ? 'Rental' : 'Purchase'}</span>
                            ...
                            {b.type === 'rent' && <p className="text-xs text-muted-foreground">{b.startDate} → {b.endDate}</p>}
                            <p className="font-bold text-primary">₹{b.amount.toLocaleString('en-IN')}</p>
                          </div>
                          {status === 'pending' &&
                      <div className="flex gap-2">
                              <button onClick={() => updateBooking(b.id, 'accepted')} className="flex items-center gap-1.5 px-3 py-2 rounded-xl bg-success text-white text-xs font-bold hover:bg-success/90 transition-colors">
                                <CheckCircle size={13} /> Accept
                              </button>
                              <button onClick={() => updateBooking(b.id, 'declined')} className="flex items-center gap-1.5 px-3 py-2 rounded-xl bg-danger text-white text-xs font-bold hover:bg-danger/90 transition-colors">
                                <XCircle size={13} /> Decline
                              </button>
                            </div>
                      }
        ...
        <div className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              {[
            { label: 'Total Earned', value: `₹${totalEarnings.toLocaleString('en-IN')}`, sub: 'All time', color: 'text-success' },
            { label: 'This Month', value: '₹15,000', sub: 'August 2026', color: 'text-primary' },
            { label: 'Pending Payout', value: '₹8,500', sub: 'Processing', color: 'text-warning' }].
            map((s) =>
            ...
            <div className="bg-card rounded-2xl border border-border p-5">
              <div className="flex items-center justify-between mb-5">
                <h2 className="font-bold text-base text-foreground flex items-center gap-2"><BarChart2 size={16} className="text-primary" /> Monthly Earnings</h2>
              </div>
              <div className="flex items-end gap-3 h-40">
                {EARNINGS_MONTHLY.map((e) =>
              <div key={e.month} className="flex-1 flex flex-col items-center gap-1">
                    <span className="text-xs font-bold text-primary">₹{(e.amount / 1000).toFixed(0)}k</span>
                    <div className="w-full rounded-t-lg gradient-green transition-all" style={{ height: `${e.amount / maxEarning * 100}%`, minHeight: 8 }} />
            ...
            <div className="bg-card rounded-2xl border border-border p-5">
              <h2 className="font-bold text-base text-foreground mb-4 flex items-center gap-2"><TrendingUp size={16} className="text-primary" /> Earnings by Listing</h2>
              <div className="space-y-3">
                {listings.filter((l) => l.earnings > 0).map((l) =>
              <div key={l.id} className="flex items-center gap-3">
                    <img src={l.image} alt={l.name} className="w-10 h-10 rounded-lg object-cover shrink-0" />
                    <div className="flex-1 min-w-0">
                      <p className="font-semibold text-sm text-foreground truncate">{l.name}</p>
                      <div className="w-full bg-muted rounded-full h-1.5 mt-1">
                        <div className="gradient-green h-1.5 rounded-full" style={{ width: `${l.earnings / totalEarnings * 100}%` }} />
                      </div>
                    </div>
                    <span className="font-bold text-sm text-success shrink-0">₹{l.earnings.toLocaleString('en-IN')}</span>
                  </div>
              )}
              </div>
            </div>
