...
const SECTIONS = [
  {
    ...
    color: 'border-primary/30 bg-secondary/40',
    steps: [
      ...
      <div className="max-w-screen-2xl mx-auto px-4 lg:px-8 xl:px-10 2xl:px-16">
        <div className="text-center mb-10">
          ...
          <p className="text-muted-foreground max-w-xl mx-auto">
            Three simple workflows for every farming need — designed for Indian farmers
          </p>
        </div>
              ...
              <ol className="space-y-3">
                {section?.steps?.map((step, i) => (
                  <li key={`hiw-${section?.id}-step-${i + 1}`} className="flex items-start gap-3">
                    <span className="w-5 h-5 rounded-full gradient-green text-white text-xs font-bold flex items-center justify-center shrink-0 mt-0.5">
                      {i + 1}
                    </span>
