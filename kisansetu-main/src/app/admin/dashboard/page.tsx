...
const USERS: User[] = [
  ...
  { id: 'u5', name: 'Anita Sharma', phone: '+91 54321 55555', role: 'supplier', joined: '2026-04-01', status: 'active', bookings: 18 },
];
...
const SUPPLIER_APPROVALS: SupplierApproval[] = [
  ...
  { id: 's4', name: 'Meena Patel', phone: '+91 66554 33221', equipment: 'Sprayer', submitted: '2026-08-05', status: 'rejected', docs: false },
];
...
const EQUIPMENT_APPROVALS: EquipmentApproval[] = [
  ...
  { id: 'eq4', name: 'Old Sprayer 2018', supplier: 'Meena Patel', category: 'Sprayer', rentPerDay: 800, submitted: '2026-08-06', status: 'rejected' },
];
...
export default function AdminDashboard() {
  ...
  function updateSupplier(id: string, status: 'approved' | 'rejected') {
    setSuppliers(prev => prev.map(s => s.id === id ? { ...s, status } : s));
  }
  function updateEquipment(id: string, status: 'approved' | 'rejected') {
      ...
      <main className="max-w-screen-xl mx-auto px-4 lg:px-8 py-6">
        <div className="flex items-center justify-between mb-6">
          <div>
            ...
            <p className="text-sm text-muted-foreground mt-0.5">KisanSetu Platform Management</p>
          </div>
          <div className="flex items-center gap-2 text-xs font-semibold text-success bg-success/10 px-3 py-1.5 rounded-full">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="bg-card rounded-2xl border border-border p-5">
                ...
                <div className="space-y-3">
                  {[
                    { label: 'Farmers Registered', value: users.filter(u => u.role === 'farmer').length, total: users.length },
                    { label: 'Suppliers Active', value: users.filter(u => u.role === 'supplier' && u.status === 'active').length, total: users.filter(u => u.role === 'supplier').length },
                    { label: 'Equipment Approved', value: equipment.filter(e => e.status === 'approved').length, total: equipment.length },
                  ].map(s => (
                    <div key={s.label}>
                      <div className="flex justify-between text-xs mb-1">
                        <span className="text-muted-foreground">{s.label}</span>
                        <span className="font-bold text-foreground">{s.value}/{s.total}</span>
                      </div>
                      <div className="w-full bg-muted rounded-full h-1.5">
                        <div className="gradient-green h-1.5 rounded-full" style={{ width: `${(s.value / s.total) * 100}%` }} />
                      </div>
                    </div>
                  ))}
                </div>
              ...
              <div className="bg-card rounded-2xl border border-border p-5">
                ...
                <div className="space-y-2.5">
                  {[
                    { label: 'Equipment Rentals', amount: 280000, pct: 67 },
                    { label: 'Equipment Sales', amount: 95000, pct: 23 },
                    { label: 'Labour Bookings', amount: 42000, pct: 10 },
                  ].map(r => (
                    <div key={r.label} className="flex items-center gap-3">
                      <div className="flex-1">
                        <div className="flex justify-between text-xs mb-1">
                          <span className="text-muted-foreground">{r.label}</span>
                          <span className="font-bold text-foreground">₹{(r.amount / 1000).toFixed(0)}k</span>
                        </div>
                        <div className="w-full bg-muted rounded-full h-1.5">
                          <div className="gradient-green h-1.5 rounded-full" style={{ width: `${r.pct}%` }} />
                        </div>
                      </div>
          ...
          <div className="space-y-4">
            <div className="flex items-center gap-3">
              <div className="relative flex-1 max-w-sm">
                ...
                <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search users..." className="input-field pl-9 text-sm w-full" />
              </div>
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  <thead className="bg-muted/50">
                    <tr>
                      {['Name', 'Phone', 'Role', 'Joined', 'Bookings', 'Status', 'Action'].map(h => (
                      ...
                      <tr key={u.id} className="hover:bg-muted/30 transition-colors">
                        ...
                        <td className="px-4 py-3">
                          <div className="flex items-center gap-2">
                            ...
                            <button onClick={() => toggleUserStatus(u.id)} className={`p-1.5 rounded-lg transition-colors ${u.status === 'active' ? 'hover:bg-danger/10 text-danger' : 'hover:bg-success/10 text-success'}`}>
                        ...
                        <div className="flex items-start justify-between gap-3 flex-wrap">
                          <div className="space-y-1">
                            ...
                            <div className="flex items-center gap-2 mt-1">
                              <span className={`px-2 py-0.5 rounded-full text-xs font-bold ${s.docs ? 'bg-success/15 text-success' : 'bg-danger/15 text-danger'}`}>
                                {s.docs ? '✓ Docs Submitted' : '✗ Docs Missing'}
                              </span>
                            </div>
                          </div>
                          {status === 'pending' && (
                            <div className="flex gap-2">
                              <button onClick={() => updateSupplier(s.id, 'approved')} className="flex items-center gap-1.5 px-3 py-2 rounded-xl bg-success text-white text-xs font-bold hover:bg-success/90 transition-colors">
                                <CheckCircle size={13} /> Approve
                              </button>
                              <button onClick={() => updateSupplier(s.id, 'rejected')} className="flex items-center gap-1.5 px-3 py-2 rounded-xl bg-danger text-white text-xs font-bold hover:bg-danger/90 transition-colors">
                                <XCircle size={13} /> Reject
                              </button>
                            </div>
                          )}
                          {status !== 'pending' && (
                            <span className={`flex items-center gap-1.5 text-xs font-bold ${status === 'approved' ? 'text-success' : 'text-danger'}`}>
                              {status === 'approved' ? <><CheckCircle size={14} /> Approved</> : <><XCircle size={14} /> Rejected</>}
                            </span>
                          )}
                        </div>
                      </div>
                      ...
                      <div key={e.id} className="bg-card rounded-2xl border border-border p-4">
                        <div className="flex items-start justify-between gap-3 flex-wrap">
                          <div className="space-y-1">
                            ...
                            <p className="text-xs text-muted-foreground">Supplier: {e.supplier} · Category: {e.category}</p>
                            <p className="text-xs text-muted-foreground">Rent: ₹{e.rentPerDay.toLocaleString('en-IN')}/day · Submitted: {e.submitted}</p>
                          </div>
                          {status === 'pending' && (
                            <div className="flex gap-2">
                              <button onClick={() => updateEquipment(e.id, 'approved')} className="flex items-center gap-1.5 px-3 py-2 rounded-xl bg-success text-white text-xs font-bold hover:bg-success/90 transition-colors">
                                <CheckCircle size={13} /> Approve
                              </button>
                              <button onClick={() => updateEquipment(e.id, 'rejected')} className="flex items-center gap-1.5 px-3 py-2 rounded-xl bg-danger text-white text-xs font-bold hover:bg-danger/90 transition-colors">
                                <XCircle size={13} /> Reject
                              </button>
                            </div>
                          )}
        ...
        {tab === 'payments' && (
          <div className="space-y-4">
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
              {[
                { label: 'Total Volume', value: '₹4,17,000', color: 'text-primary' },
                { label: 'Platform Revenue', value: '₹8,340', color: 'text-success' },
                { label: 'Pending Payouts', value: '₹23,500', color: 'text-warning' },
                { label: 'Refunds Issued', value: '₹5,200', color: 'text-danger' },
              ].map(s => (
                <div key={s.label} className="bg-card rounded-2xl border border-border p-4">
            ...
            <div className="bg-card rounded-2xl border border-border overflow-hidden">
              <div className="px-5 py-4 border-b border-border">
                <h2 className="font-bold text-sm text-foreground">Recent Transactions</h2>
              </div>
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  <thead className="bg-muted/50">
                    <tr>
                      {['ID', 'User', 'Type', 'Amount', 'Date', 'Status'].map(h => (
                        <th key={h} className="text-left px-4 py-3 text-xs font-bold text-muted-foreground uppercase tracking-wider">{h}</th>
                      ))}
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-border">
                    {[
                      { id: 'pay_001', user: 'Suresh Yadav', type: 'Rental', amount: 7500, date: '2026-08-10', status: 'success' },
                      { id: 'pay_002', user: 'Priya Deshmukh', type: 'Purchase', amount: 95000, date: '2026-08-09', status: 'success' },
                      { id: 'pay_003', user: 'Mohan Kulkarni', type: 'Rental', amount: 3500, date: '2026-08-08', status: 'refunded' },
                      { id: 'pay_004', user: 'Anita Sharma', type: 'Labour', amount: 1200, date: '2026-08-07', status: 'success' },
                      { id: 'pay_005', user: 'Ramesh Patil', type: 'Rental', amount: 5000, date: '2026-08-06', status: 'pending' },
                    ].map(t => (
                      <tr key={t.id} className="hover:bg-muted/30 transition-colors">
                        <td className="px-4 py-3 font-mono text-xs text-muted-foreground">{t.id}</td>
                        <td className="px-4 py-3 font-semibold text-foreground">{t.user}</td>
                        ...
                        <td className="px-4 py-3">
                          <span className={`px-2 py-0.5 rounded-full text-xs font-bold ${t.status === 'success' ? 'bg-success/15 text-success' : t.status === 'refunded' ? 'bg-warning/15 text-warning' : 'bg-muted text-muted-foreground'}`}>{t.status}</span>
                        </td>
                      </tr>
                    ))}
                  </tbody>
