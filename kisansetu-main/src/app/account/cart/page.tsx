...
{
  id: 'c-001', name: 'DAP Fertilizer (50 Kg)',
...
{
  id: 'c-002', name: 'Paddy Seeds — Hybrid IR-64',
  ...
  price: 950, mrp: 1100, unit: 'per kg', seller: 'AgroMart Pune', quantity: 3, inStock: true, category: 'Seeds'
},
{
  id: 'c-003', name: 'Organic Pesticide (1 Ltr)',
  ...
  price: 320, mrp: 380, unit: 'per bottle', seller: 'GreenShield Agro', quantity: 1, inStock: true, category: 'Pesticides'
}];
...
export default function CartPage() {
  ...
  function updateQty(id: string, delta: number) {
    setItems((p) => p.map((item) => item.id === id ? { ...item, quantity: Math.max(1, item.quantity + delta) } : item));
  }
  ...
  function applyCoupon() {
    if (coupon.toUpperCase() === 'KISAN10') {
      ...
      setCouponError('');
    } else if (coupon.toUpperCase() === 'MONSOON20') {
      setAppliedCoupon('MONSOON20');
      setCouponError('');
    } else {
      setCouponError('Invalid coupon code. Try KISAN10 or MONSOON20');
      setAppliedCoupon('');
    }
  }
        ...
        <main className="max-w-lg mx-auto px-4 py-16 text-center">
          <div className="card-base p-10">
            ...
            <div className="bg-muted/50 rounded-xl p-4 text-left mb-6 space-y-2">
              ...
              <div className="flex justify-between text-sm"><span className="text-muted-foreground">Order ID</span><span className="font-semibold text-foreground font-tabular text-xs">{paymentResult.orderId}</span></div>
            ...
            <div className="flex-1 min-w-0 space-y-3">
                    ...
                    <div className="flex gap-4">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-start justify-between gap-2">
                          ...
                          <button suppressHydrationWarning onClick={() => removeItem(item.id)} className="p-1.5 rounded-lg hover:bg-danger-bg text-muted-foreground hover:text-danger transition-colors shrink-0">
                            <Trash2 size={15} />
                          </button>
                        </div>
                        ...
                        <div className="flex items-center justify-between mt-3 flex-wrap gap-2">
                          ...
                          <div className="flex items-center gap-2">
                            <button suppressHydrationWarning onClick={() => updateQty(item.id, -1)} disabled={item.quantity <= 1} className="w-7 h-7 rounded-lg border border-border flex items-center justify-center hover:bg-muted transition-colors disabled:opacity-40 disabled:cursor-not-allowed">
                              <Minus size={13} />
                            </button>
                            <span className="w-8 text-center font-bold text-sm font-tabular">{item.quantity}</span>
                            <button suppressHydrationWarning onClick={() => updateQty(item.id, 1)} className="w-7 h-7 rounded-lg border border-border flex items-center justify-center hover:bg-muted transition-colors">
                              <Plus size={13} />
                            </button>
              ...
              <div className="card-base p-4">
                <div className="flex items-center gap-2 mb-3">
                  ...
                  <h3 className="font-semibold text-sm text-foreground">Apply Coupon</h3>
                </div>
                {appliedCoupon ?
              <div className="flex items-center justify-between bg-success-bg border border-success/30 rounded-xl px-4 py-3">
                    <div className="flex items-center gap-2">
                      <CheckCircle size={16} className="text-success" />
                      <span className="font-bold text-sm text-success">{appliedCoupon}</span>
                      <span className="text-xs text-success">— {appliedCoupon === 'KISAN10' ? '10%' : '20%'} discount applied!</span>
                    </div>
                    <button suppressHydrationWarning onClick={removeCoupon} className="text-xs text-danger font-medium hover:underline">Remove</button>
                  </div> :
              ...
              <div className="card-base p-4 bg-secondary/40">
                <div className="flex items-center gap-2 text-sm text-foreground">
                  ...
                  {deliveryFee === 0 ?
                <span><span className="font-semibold text-success">Free delivery</span> on this order! (Orders above ₹2,000)</span> :

                <span>Add <span className="font-semibold text-primary">₹{(2000 - subtotal).toLocaleString('en-IN')}</span> more for free delivery</span>
                }
                </div>
              </div>
            </div>
              ...
              <div className="card-base p-5 sticky top-20">
                ...
                <div className="space-y-3 mb-4">
                  <div className="flex justify-between text-sm">
                    ...
                    <span className="font-semibold font-tabular">₹{subtotal.toLocaleString('en-IN')}</span>
                  </div>
                  {savings > 0 &&
                <div className="flex justify-between text-sm">
                      <span className="text-muted-foreground">Product Discount</span>
                      <span className="font-semibold text-success font-tabular">−₹{savings.toLocaleString('en-IN')}</span>
                    </div>
                }
                  {couponDiscount > 0 &&
                <div className="flex justify-between text-sm">
                      <span className="text-muted-foreground">Coupon ({appliedCoupon})</span>
                      <span className="font-semibold text-success font-tabular">−₹{couponDiscount.toLocaleString('en-IN')}</span>
                    </div>
                }
                  <div className="flex justify-between text-sm">
                    <span className="text-muted-foreground">Delivery Fee</span>
                    <span className={`font-semibold font-tabular ${deliveryFee === 0 ? 'text-success' : ''}`}>{deliveryFee === 0 ? 'FREE' : `₹${deliveryFee}`}</span>
                  </div>
                </div>
                ...
                <div className="border-t border-border pt-3 mb-5">
                  <div className="flex justify-between">
                    ...
                    <span className="font-extrabold text-xl text-primary font-tabular">₹{total.toLocaleString('en-IN')}</span>
                  </div>
                  {savings + couponDiscount > 0 &&
                <p className="text-xs text-success mt-1 text-right">You save ₹{(savings + couponDiscount).toLocaleString('en-IN')} on this order!</p>
                }
                </div>

                {paymentState === 'error' &&
              <div className="bg-danger-bg border border-danger/30 rounded-xl px-4 py-3 mb-4 text-sm text-danger">
                ...
                description="KisanSetu Agri Supplies Order"
                buttonText={`Pay ₹${total.toLocaleString('en-IN')}`}
                prefill={{ name: 'Johan Rame', contact: '+919876543210' }}
                notes={{ source: 'cart', items: items.map((i) => i.name).join(', ') }}
                onSuccess={handlePaymentSuccess}
                onError={handlePaymentError}
                onDismiss={() => setPaymentState('idle')}
                className="w-full flex items-center justify-center gap-2 py-3.5 px-6 rounded-xl font-bold text-base gradient-green text-white hover:opacity-90 transition-all btn-press disabled:opacity-60 disabled:cursor-not-allowed" />
                ...
                <div className="mt-4 pt-4 border-t border-border">
                  <p className="text-xs text-muted-foreground text-center mb-3">Test card: <span className="font-mono font-semibold">4100 2800 0000 1007</span></p>
                  <Link href="/agri" className="flex items-center justify-center gap-1.5 text-sm font-medium text-primary hover:underline transition-colors">
