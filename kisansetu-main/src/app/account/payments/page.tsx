...
interface Transaction {
  id: string; paymentId: string; type: 'payment' | 'refund';
  description: string; amount: number; date: string;
  status: TxStatus; method: string; bookingId?: string;
}
...
const TRANSACTIONS: Transaction[] = [
  ...
  { id: 't8', paymentId: 'pay_QxR8901234', type: 'payment', description: 'Sprayer Machine — 1 day rental', amount: 800, date: '2026-07-05', status: 'success', method: 'UPI', bookingId: 'BKG12305' },
];
    ...
    <div className="min-h-screen bg-background">
      ...
      <main className="max-w-screen-lg mx-auto px-4 lg:px-8 py-6">
        ...
        <div className="flex items-center gap-2 text-sm text-muted-foreground mb-6">
          ...
          <Link href="/account/profile" className="hover:text-primary transition-colors">Account</Link>
          <ChevronRight size={14} />
        ...
        <div className="flex items-center justify-between mb-6">
          ...
          <button
            onClick={() => {
              const headers = ['Date', 'Description', 'Payment ID', 'Method', 'Type', 'Amount', 'Status'];
              ...
              a.href = url;
              a.download = 'kisansetu-payments.csv';
              a.click();
              URL.revokeObjectURL(url);
            }}
            className="flex items-center gap-2 px-3 py-2 rounded-xl border border-border text-sm font-medium hover:bg-muted transition-colors">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
          <div className="bg-card rounded-2xl border border-border p-4">
            ...
            <p className="text-xs text-muted-foreground mt-0.5">{TRANSACTIONS.filter(t => t.type === 'payment' && t.status === 'success').length} transactions</p>
          </div>
          <div className="bg-card rounded-2xl border border-border p-4">
            <div className="flex items-center gap-3 mb-2">
            ...
            <p className="text-xs text-muted-foreground mt-0.5">{TRANSACTIONS.filter(t => t.type === 'refund').length} refunds</p>
          </div>
          <div className="bg-card rounded-2xl border border-border p-4">
            <div className="flex items-center gap-3 mb-2">
            ...
            <p className="text-xs text-muted-foreground mt-0.5">transactions processing</p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-3 mb-4 flex-wrap">
          ...
          <div className="relative flex-1 min-w-[200px]">
            ...
            <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search by description or payment ID..." className="input-field pl-9 text-sm w-full" />
          </div>
        </div>
                ...
                <div className="flex items-start gap-4">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-start justify-between gap-3 flex-wrap">
                      <div>
                        <p className="font-semibold text-sm text-foreground leading-snug">{t.description}</p>
                        <div className="flex items-center gap-3 mt-1 flex-wrap">
                          <span className="text-xs text-muted-foreground font-mono">{t.paymentId}</span>
                          {t.bookingId && (
                            <Link href="/account/bookings" className="text-xs text-primary hover:underline font-semibold">#{t.bookingId}</Link>
                          )}
                        </div>
                      ...
                      <div className="text-right shrink-0">
                        ...
                        <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-bold mt-1 ${sc.color}`}>
        ...
        <div className="mt-6 bg-muted/40 rounded-2xl p-4 flex gap-3">
          ...
          <div>
            <p className="font-semibold text-sm text-foreground mb-1">Refund Policy</p>
            <p className="text-xs text-muted-foreground leading-relaxed">Security deposits are refunded within 48 hours of equipment return and inspection. Booking cancellations made 24+ hours before start date receive a full refund. For disputes, contact support at 1800-123-KISAN.</p>
          </div>
        </div>
      </main>
      <Footer />
