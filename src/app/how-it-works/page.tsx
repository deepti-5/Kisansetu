      ...
      <main>
        <section className="bg-gradient-to-br from-primary/5 via-background to-accent/5 py-16 px-4">
          <div className="max-w-screen-lg mx-auto text-center">
            ...
            <h1 className="text-4xl lg:text-5xl font-extrabold text-foreground leading-tight mb-4">Farm smarter.<br /><span className="text-primary">Rent, hire, grow.</span></h1>
            <p className="text-lg text-muted-foreground max-w-2xl mx-auto mb-8">KisanSetu connects farmers with equipment suppliers, skilled labour, and agri supplies — all in one platform. No brokers, no delays.</p>
            <div className="flex items-center justify-center gap-3 flex-wrap">
              <Link href="/equipment-listing-page" className="btn-primary px-6 py-3 rounded-xl font-bold flex items-center gap-2"><Tractor size={18} /> Browse Equipment</Link>
              <Link href="/supplier/onboarding" className="btn-secondary px-6 py-3 rounded-xl font-bold flex items-center gap-2"><Package size={18} /> Become a Supplier</Link>
            </div>
          </div>
        </section>
              ...
              {FARMER_STEPS?.map((s, i) => (
                <div key={s?.step} className="relative">
                  <div className="bg-card rounded-2xl border border-border p-5 h-full">
                    <div className={`w-12 h-12 rounded-2xl ${s?.color} flex items-center justify-center mb-4`}><s.icon size={22} /></div>
                  ...
                  {i < FARMER_STEPS?.length - 1 && <div className="hidden lg:flex absolute top-1/2 -right-3 -translate-y-1/2 z-10"><ChevronRight size={20} className="text-muted-foreground/40" /></div>}
                </div>
              ))}
            </div>
          </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
              {SUPPLIER_STEPS?.map((s, i) => (
                <div key={s?.step} className="relative">
                  ...
                  {i < SUPPLIER_STEPS?.length - 1 && <div className="hidden lg:flex absolute top-1/2 -right-3 -translate-y-1/2 z-10"><ChevronRight size={20} className="text-muted-foreground/40" /></div>}
                </div>
              ))}
            </div>
          </div>
        ...
        <section className="py-16 px-4">
          <div className="max-w-screen-lg mx-auto">
            <div className="text-center mb-10"><h2 className="text-3xl font-extrabold text-foreground">Why farmers trust KisanSetu</h2></div>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
              {FEATURES?.map(f => (
                <div key={f?.title} className="flex gap-4 bg-card rounded-2xl border border-border p-5">
                  <div className="w-10 h-10 rounded-xl bg-primary/10 flex items-center justify-center shrink-0"><f.icon size={18} className="text-primary" /></div>
                  <div><h3 className="font-bold text-sm text-foreground mb-1">{f?.title}</h3><p className="text-xs text-muted-foreground leading-relaxed">{f?.desc}</p></div>
                </div>
              ))}
            </div>
          </div>
        ...
        <section className="py-12 px-4 bg-foreground text-white">
          <div className="max-w-screen-lg mx-auto">
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-6 text-center">
              {[{ value: '50,000+', label: 'Farmers Served' }, { value: '2,800+', label: 'Equipment Listed' }, { value: '18 States', label: 'Pan India Coverage' }, { value: '₹12 Cr+', label: 'Transactions Processed' }]?.map(s => (
                <div key={s?.label}><p className="text-3xl font-extrabold text-accent">{s?.value}</p><p className="text-sm text-white/70 mt-1">{s?.label}</p></div>
              ))}
            </div>
          </div>
        </section>
        ...
        <section className="py-16 px-4">
          <div className="max-w-screen-md mx-auto">
            <div className="text-center mb-10"><h2 className="text-3xl font-extrabold text-foreground flex items-center justify-center gap-2"><HelpCircle size={28} className="text-primary" /> Frequently Asked Questions</h2></div>
            <div className="space-y-3">
              {FAQS?.map((faq, i) => (
                <details key={i} className="bg-card rounded-2xl border border-border group">
                  <summary className="flex items-center justify-between px-5 py-4 cursor-pointer font-semibold text-sm text-foreground list-none">{faq?.q}<ChevronRight size={16} className="text-muted-foreground group-open:rotate-90 transition-transform shrink-0 ml-3" /></summary>
