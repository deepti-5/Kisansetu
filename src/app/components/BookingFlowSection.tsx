        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-1 card-base p-6">
            ...
            <div className="space-y-3 text-sm">
              <div className="flex justify-between"><span className="text-muted-foreground">Equipment</span><span className="font-semibold text-foreground">Mahindra Yuvo Tractor</span></div>
              <div className="flex justify-between"><span className="text-muted-foreground">Start</span><span className="font-semibold font-tabular">12 May 2025 · 09:00 AM</span></div>
              <div className="flex justify-between"><span className="text-muted-foreground">End</span><span className="font-semibold font-tabular">15 May 2025 · 06:00 PM</span></div>
              <div className="flex justify-between"><span className="text-muted-foreground">Duration</span><span className="font-semibold">3 Days</span></div>
              <div className="border-t border-border pt-3 space-y-2">
                <div className="flex justify-between"><span className="text-muted-foreground">Rental Amount</span><span className="font-tabular">₹7,500</span></div>
                <div className="flex justify-between"><span className="text-muted-foreground">Security Deposit</span><span className="font-tabular text-warning">₹10,000</span></div>
                <div className="flex justify-between"><span className="text-muted-foreground">Platform Fee</span><span className="font-tabular">₹150</span></div>
              </div>
              ...
              <button suppressHydrationWarning className="w-full btn-primary py-3 text-sm mt-2">Continue to Payment</button>
            </div>
          </div>
          ...
          <div className="lg:col-span-1 card-base p-6">
            ...
            <div className="space-y-2">
              {PAYMENT_METHODS?.map((method) => (
                <div
                  key={`pay-${method?.name}`}
                  className={`flex items-center gap-3 p-3 rounded-lg border transition-colors cursor-pointer hover:border-primary hover:bg-secondary/50 ${method?.popular ? 'border-primary bg-secondary/30' : 'border-border'}`}
                >
                  <span className="text-xl">{method?.icon}</span>
                  <span className="text-sm font-medium text-foreground flex-1">{method?.name}</span>
                  {method?.popular && <span className="badge-green text-xs">Popular</span>}
                </div>
              ))}
            </div>
            <button suppressHydrationWarning className="w-full btn-accent py-3 text-sm font-bold mt-4">Pay ₹17,650</button>
          </div>
        </div>
      </div>
    </section>
