...
export default function HeroSection() {
  ...
  function detectLocation() {
    ...
    navigator.geolocation.getCurrentPosition(
      ...
      () => { setLocation('Pune, Maharashtra'); setLocationLoading(false); },
      { timeout: 8000 }
    );
  }
  ...
  function handleVoice() {
    setVoiceActive(true);
    setTimeout(() => { setQuery('Mahindra Tractor near Pune'); setVoiceActive(false); }, 2000);
  }
  ...
  function handleCameraUpload(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    setTimeout(() => { setCameraLoading(false); setCameraOpen(false); setQuery('Rotavator (identified from image)'); }, 2500);
  }
              ...
              <div className="flex items-center gap-2">
                ...
                <div className="flex items-center gap-1">
                  <button
                    suppressHydrationWarning
                    onClick={handleVoice}
                    className={`p-2.5 rounded-xl transition-all duration-150 ${voiceActive ? 'bg-danger text-white animate-pulse' : 'bg-muted hover:bg-secondary text-muted-foreground hover:text-primary'}`}
                    title="Voice search"
                  >
                    <Mic size={18} />
                  </button>
