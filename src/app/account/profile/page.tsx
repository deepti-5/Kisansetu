...
import { User, Phone, Mail, MapPin, Camera, Save, ChevronRight, ShoppingBag, CreditCard, Heart, Bell, Shield, LogOut, Edit3, CheckCircle } from 'lucide-react';
...
export default function ProfilePage() {
  ...
  const [profile, setProfile] = useState<ProfileData>({ name: 'Johan Rame', phone: '+91 98765 43210', email: 'johan.rame@email.com', village: 'Hadapsar', district: 'Pune', state: 'Maharashtra', pincode: '411028', farmSize: '5 acres', crops: 'Wheat, Sugarcane, Onion' });
  const [draft, setDraft] = useState<ProfileData>(profile);

  function handleSave() { setProfile(draft); setEditing(false); setSaved(true); toast.success('Profile updated successfully!'); setTimeout(() => setSaved(false), 3000); }
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          <div className="space-y-4">
            <div className="bg-card rounded-2xl border border-border p-6 text-center">
              ...
              <div className="flex items-center justify-center gap-1.5 mt-2"><span className="w-2 h-2 rounded-full bg-success" /><span className="text-xs text-success font-semibold">Verified Farmer</span></div>
              ...
              <div className="flex items-center justify-between mb-6">
                ...
                {!editing ? (
                  ...
                  <div className="flex gap-2">
                    ...
                    <button onClick={handleSave} className="flex items-center gap-2 px-4 py-2 rounded-xl btn-primary text-sm font-semibold"><Save size={14} /> Save Changes</button>
                  </div>
                )}
              </div>
              {saved && <div className="flex items-center gap-2 bg-success/10 border border-success/30 rounded-xl px-4 py-3 mb-4 text-sm text-success font-semibold"><CheckCircle size={16} /> Profile updated successfully!</div>}
                  ...
                  <div key={f.key} className={f.key === 'crops' ? 'sm:col-span-2' : ''}>
                    ...
                    {editing ? (
                      <div className="relative">
                        ...
                        <input type={f.type || 'text'} value={draft[f.key]} onChange={e => setDraft(prev => ({ ...prev, [f.key]: e.target.value }))} placeholder={f.placeholder} className="input-field pl-9 text-sm w-full" />
                      </div>
                    ) : (
