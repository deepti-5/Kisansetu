...
export default function ProfilePage() {
  ...
  function handleSave() { setProfile(draft); setEditing(false); setSaved(true); toast.success(t('profileUpdated')); setTimeout(() => setSaved(false), 3000); }
          ...
          <div className="space-y-4">
            <div className="bg-card rounded-2xl border border-border p-6 text-center">
              ...
              <div className="grid grid-cols-3 gap-2 mt-4 pt-4 border-t border-border">
                {[{ labelKey: 'bookings', value: 12 }, { labelKey: 'reviews', value: 8 }, { labelKey: 'wishlist', value: 5 }].map(s => (
              ...
              <div className="flex items-center justify-between mb-6">
                ...
                {!editing ? (
                  ...
                  <div className="flex gap-2">
                    ...
                    <button onClick={handleSave} className="flex items-center gap-2 px-4 py-2 rounded-xl btn-primary text-sm font-semibold"><Save size={14} /> {t('saveChanges')}</button>
                  </div>
                )}
              </div>
                  ...
                  <div key={f.key} className={f.key === 'crops' ? 'sm:col-span-2' : ''}>
                    ...
                    {editing ? (
                      <div className="relative">
                        ...
                        <input type={f.type || 'text'} value={draft[f.key]} onChange={e => setDraft(prev => ({ ...prev, [f.key]: e.target.value }))} placeholder={t(f.placeholderKey)} className="input-field pl-9 text-sm w-full" />
                      </div>
                    ) : (
