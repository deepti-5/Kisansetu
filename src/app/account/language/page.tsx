...
export default function LanguagePage() {
  ...
  useEffect(() => {
    ...
    if (stored) { setSelected(stored); setSavedLang(stored); }
  }, []);

  function handleSave() {
    localStorage.setItem('kisansetu_language', selected);
    document.documentElement.lang = selected;
    setSavedLang(selected); setSaved(true);
    ...
    <div className="min-h-screen bg-background">
      ...
      <main className="max-w-2xl mx-auto px-4 py-8">
        ...
        <div className="flex items-center gap-4 mb-8">
          ...
          <div><h1 className="text-2xl font-extrabold text-foreground">Choose Language</h1><p className="text-sm text-muted-foreground">Select your preferred language for the KisanSetu platform</p></div>
        </div>
        {savedLang && (
          <div className="mb-5 flex items-center gap-2 bg-primary/5 border border-primary/20 rounded-xl px-4 py-3">
            <Check size={16} className="text-primary shrink-0" />
            <p className="text-sm text-foreground">Current language: <span className="font-bold text-primary">{LANGUAGES.find((l) => l.code === savedLang)?.nativeName || 'English'}</span></p>
          </div>
        )}
        ...
        {!saved && <p className="text-center text-xs text-muted-foreground mt-4">Language preference is saved to your device.</p>}
      </main>
      <Footer />
    </div>
  );
