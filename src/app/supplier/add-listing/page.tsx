      ...
      <div className="min-h-screen bg-background">
        ...
        <main className="max-w-screen-sm mx-auto px-4 py-16 text-center">
          ...
          <div className="flex gap-3 justify-center">
            ...
            <button onClick={() => {
              setSubmitted(false);
              setForm({ name: '', category: '', brand: '', model: '', year: '', condition: 'Good', description: '', rentPerDay: '', buyPrice: '', deposit: '', forRent: true, forSale: false, location: '', pincode: '', latitude: '', longitude: '', address: '', rentalType: 'tractor-only', driver: { name: '', phone: '', experience: '', availability: '', serviceArea: '' } });
            }} className="btn-secondary px-6 py-3">Add Another</button>
          </div>
        </main>
        <Footer />
      </div>
    ...
    <div className="min-h-screen bg-background">
      ...
      <main className="max-w-screen-lg mx-auto px-4 lg:px-8 py-6">
        ...
        <form onSubmit={handleSubmit} className="space-y-6 max-w-2xl">
          ...
          {/* Rental Type — shown for Tractor category or tractor in name */}
          {(isTractor || form.forRent) && (
            <div className="bg-card rounded-2xl border border-border p-6 space-y-4">
              ...
              <p className="text-sm text-muted-foreground">Choose whether you're renting the tractor alone or with an operator/driver.</p>

              <div className="grid grid-cols-2 gap-3">
                <button
                  type="button"
                  onClick={() => update('rentalType', 'tractor-only')}
                  className={`relative flex flex-col items-center gap-2 p-4 rounded-xl border-2 transition-all ${form.rentalType === 'tractor-only' ? 'border-primary bg-primary/5' : 'border-border hover:border-primary/40'}`}
                >
                  <div className={`w-10 h-10 rounded-full flex items-center justify-center ${form.rentalType === 'tractor-only' ? 'bg-primary/10' : 'bg-muted'}`}>
                    <Tractor size={20} className={form.rentalType === 'tractor-only' ? 'text-primary' : 'text-muted-foreground'} />
                  </div>
                  <span className={`text-sm font-bold ${form.rentalType === 'tractor-only' ? 'text-primary' : 'text-foreground'}`}>Tractor Only</span>
                  <span className="text-xs text-muted-foreground text-center">Renter brings their own operator</span>
                  {form.rentalType === 'tractor-only' && (
                ...
                <button
                  type="button"
                  onClick={() => update('rentalType', 'tractor-with-driver')}
                  className={`relative flex flex-col items-center gap-2 p-4 rounded-xl border-2 transition-all ${form.rentalType === 'tractor-with-driver' ? 'border-primary bg-primary/5' : 'border-border hover:border-primary/40'}`}
                >
                  <div className={`w-10 h-10 rounded-full flex items-center justify-center ${form.rentalType === 'tractor-with-driver' ? 'bg-primary/10' : 'bg-muted'}`}>
                    <User size={20} className={form.rentalType === 'tractor-with-driver' ? 'text-primary' : 'text-muted-foreground'} />
                  </div>
                  <span className={`text-sm font-bold ${form.rentalType === 'tractor-with-driver' ? 'text-primary' : 'text-foreground'}`}>Tractor + Driver</span>
                  <span className="text-xs text-muted-foreground text-center">Includes experienced operator</span>
              ...
              {/* Driver Details — shown only when tractor-with-driver is selected */}
              {form.rentalType === 'tractor-with-driver' && (
                <div className="mt-2 pt-4 border-t border-border space-y-4">
                  <div className="flex items-center gap-2 mb-1">
                    <User size={15} className="text-primary" />
                    <span className="text-sm font-bold text-foreground">Driver / Operator Details</span>
                  </div>

                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                      <label className="block text-sm font-semibold text-foreground mb-1.5">Driver Name *</label>
                      <input
                        value={form.driver.name}
                        onChange={e => updateDriver('name', e.target.value)}
                        placeholder="e.g. Ramesh Kumar"
                        className="input-field w-full"
                        required={form.rentalType === 'tractor-with-driver'}
                      />
                    </div>
                    <div>
                      <label className="block text-sm font-semibold text-foreground mb-1.5">Driver Phone *</label>
                      <input
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                      ...
                      <input
                        ...
                        onChange={e => updateDriver('experience', e.target.value)}
                  ...
                  <div>
                    ...
                    <input
                      value={form.driver.serviceArea}
                      onChange={e => updateDriver('serviceArea', e.target.value)}
                      placeholder="e.g. Pune District, within 30 km radius"
                      className="input-field w-full"
                      required={form.rentalType === 'tractor-with-driver'}
                    />
                    <p className="text-xs text-muted-foreground mt-1">Describe the geographic area where the driver is available to operate.</p>
                  </div>
                </div>
              )}
            </div>
          ...
          <div className="bg-card rounded-2xl border border-border p-6 space-y-4">
            <div className="flex items-center justify-between">
              ...
              <Link
                href={`/supplier/location-picker?returnTo=/supplier/add-listing`}
            ...
            {/* Show picked location if available */}
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="block text-sm font-semibold text-foreground mb-1.5">Village / Area *</label>
                <input value={form.location} onChange={e => update('location', e.target.value)} placeholder="e.g. Hadapsar, Pune" className="input-field w-full" required />
              </div>
              <div>
                <label className="block text-sm font-semibold text-foreground mb-1.5">Pincode *</label>
                <input value={form.pincode} onChange={e => update('pincode', e.target.value)} placeholder="411028" className="input-field w-full" />
              </div>
            </div>
          </div>
