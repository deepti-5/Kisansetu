...
const STATIC_BOOKINGS: BookingReq[] = [
...
export default function SupplierHub() {
  ...
  const [bookings, setBookings] = useState<BookingReq[]>(STATIC_BOOKINGS);
    ...
    const channel = supabase
      ...
      .on(
        ...
        (payload) => {
          ...
          toast.success(`🔔 New booking request from ${newBooking.buyer_name}!`, {
            duration: 6000,
          });
          ...
          setBookings((prev) =>
            prev.map((b) =>
              b.id === updated.booking_ref
                ? {
                    ...b,
                    status: updated.booking_status === 'accepted' ? 'accepted' : updated.booking_status === 'declined' ? 'declined' : 'pending',
                    amount: updated.total_amount as number,
                  }
                : b
            )
          );
  ...
  async function handleBookingAction(bookingId: string, action: 'accepted' | 'declined') {
    ...
    if (booking.dbId) {
      ...
      try {
        await supabase.functions.invoke('send-booking-email', {
          body: {
            type: action === 'accepted' ? 'booking_approved' : 'booking_declined',
            buyerEmail: 'farmer@kisansetu.in',
    ...
    toast.success(`Booking ${action === 'accepted' ? 'accepted' : 'declined'} successfully!`);
  }
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <main className="max-w-screen-xl mx-auto px-4 lg:px-8 py-6">
        ...
        {newBookingAlert && (
          <div className="bg-success/10 border border-success/30 rounded-xl p-4 mb-4 flex items-center gap-3 animate-pulse">
            ...
            <button onClick={() => { setTab('bookings'); setNewBookingAlert(null); }} className="text-xs font-semibold text-success hover:underline shrink-0">
              View →
            </button>
          </div>
        )}
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
              ...
              <div className="bg-card rounded-2xl border border-border p-5">
                ...
                <div className="mt-3 pt-3 border-t border-border flex items-center justify-between">
                  <p className="text-xs text-muted-foreground">Projected 30-day revenue</p>
                  <p className="font-bold text-primary">₹{UPCOMING_BOOKINGS.reduce((s, b) => s + b.amount, 0).toLocaleString('en-IN')}</p>
                </div>
              </div>
            </div>
          </div>
                ...
                <div key={l.id} className="bg-card rounded-2xl border border-border p-4 flex gap-4">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-3 mt-2">
                      ...
                      <span className="text-xs text-warning">{l.rating}★</span>
                    </div>
                  ...
                  <div className="flex items-start justify-between gap-3 flex-wrap">
                    ...
                    {b.status === 'pending' && (
                      <div className="flex items-center gap-2 shrink-0">
                        <button onClick={() => handleBookingAction(b.id, 'accepted')} className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-success text-white text-xs font-semibold hover:bg-success/90 transition-colors">
                          <CheckCircle size={13} /> Accept
                        </button>
                        <button onClick={() => handleBookingAction(b.id, 'declined')} className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-danger/10 text-danger text-xs font-semibold hover:bg-danger/20 transition-colors">
          ...
          <div className="space-y-5">
            <div className="grid grid-cols-2 lg:grid-cols-3 gap-4">
              {[
                ...
                { label: 'This Month', value: '₹78,800', color: 'text-primary' },
                { label: 'Pending Payout', value: '₹15,000', color: 'text-warning' }
              ].map((s) => (
                  ...
                  <div key={l.id} className="flex items-center gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="w-full bg-muted rounded-full h-1.5 mt-1">
                        <div className="bg-primary h-1.5 rounded-full" style={{ width: `${(l.earnings / totalEarnings) * 100}%` }} />
                      </div>
                    </div>
        ...
        {/* REVIEWS TAB */}
        {tab === 'reviews' && (
          <ReviewsList reviews={SAMPLE_REVIEWS} />
        )}
