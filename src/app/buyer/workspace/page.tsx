...
import { ShoppingBag, Heart, Package, User, Truck, RotateCcw, MapPin, Star, ChevronRight, Search, Bell, Calendar, Eye, Tractor, Wrench, Users, LogOut, Shield, Edit3, Save, Camera, Phone, Mail } from 'lucide-react';
    ...
    <div className="min-h-screen bg-background">
      ...
      <main className="max-w-screen-xl mx-auto px-4 lg:px-8 py-6">
        ...
        <div className="flex items-center justify-between mb-6">
          ...
          <div className="flex items-center gap-2">
            <button className="relative p-2.5 rounded-xl border border-border hover:bg-muted transition-colors">
              ...
              <span className="absolute top-1 right-1 w-4 h-4 rounded-full bg-success text-white text-[9px] font-bold flex items-center justify-center">{activeRentals.length}</span>
              }
            </button>
        ...
        <div className="flex items-center gap-1 bg-muted/50 rounded-xl p-1 mb-6 overflow-x-auto">
          ...
          <button
            ...
            className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-semibold transition-all whitespace-nowrap ${tab === t.key ? 'bg-card shadow text-primary' : 'text-muted-foreground hover:text-foreground'}`}>
              ...
              {t.key === 'saved' && savedItems.length > 0 &&
            <span className="w-5 h-5 rounded-full bg-primary/15 text-primary text-[10px] font-bold flex items-center justify-center">{savedItems.length}</span>
            }
            </button>
          )}
        </div>
        ...
        <div className="space-y-6">
            ...
            { label: 'Purchases', value: PURCHASES.length, icon: Package, color: 'text-accent', bg: 'bg-accent/10' },
          ...
          <div className="bg-success/5 border border-success/20 rounded-2xl p-4">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
              ...
              <div className="bg-card rounded-2xl border border-border p-5">
                <div className="flex items-center justify-between mb-4">
                  <h2 className="font-bold text-base text-foreground">Saved Items</h2>
                ...
                <div key={s.id} className="flex items-center gap-3">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-xs text-muted-foreground">{s.type === 'labour' ? `₹${s.rentPerDay}/day` : `₹${s.rentPerDay}/day`} · {s.rating}★</p>
                      </div>
        ...
        <div className="space-y-4">
            ...
            <button key={f} className="shrink-0 px-4 py-2 rounded-full text-sm font-semibold border border-border text-muted-foreground bg-card hover:border-primary hover:text-primary transition-all">{f}</button>
            )}
            </div>
            <div className="space-y-4">
            ...
            <div key={r.id} className="bg-card rounded-2xl border border-border p-5">
                  <div className="flex items-start gap-4">
                    ...
                    <button className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-warning/10 text-warning text-xs font-semibold hover:bg-warning/20 transition-colors"><RotateCcw size={13} /> Return Equipment</button>
                        ...
                        {r.status === 'returned' &&
                    <button className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-accent/10 text-accent text-xs font-semibold hover:bg-accent/20 transition-colors"><Star size={13} /> Write Review</button>
                    }
                      </div>
                    </div>
                  </div>
