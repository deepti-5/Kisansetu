  ...
  return (
    <div className="card-base overflow-hidden sticky top-20">
      ...
      <div className="divide-y divide-border max-h-[calc(100vh-180px)] overflow-y-auto scrollbar-hide">
            ...
            <div className="space-y-2">
              {CATEGORIES.filter((c) => c !== 'All').map((cat) => (
                <label key={`filter-cat-${cat}`} className="flex items-center gap-2.5 cursor-pointer group">
                  <input type="checkbox" checked={filters.categories.includes(cat)} onChange={() => toggleCategory(cat)} className="w-4 h-4 rounded accent-primary" />
        ...
        <div className="p-4">
          <button onClick={() => toggleSection('listingType')} className="flex items-center justify-between w-full mb-3">
        ...
        <div className="p-4">
          ...
          {openSections.price && (
            <div>
              ...
              <input type="range" min={0} max={10000} step={100} value={filters.maxPrice} onChange={(e) => onChange({ ...filters, maxPrice: Number(e.target.value) })} className="w-full accent-primary" />
              <div className="flex gap-2 mt-2">
                {[500, 1000, 2000, 5000].map((p) => (
                  <button key={`price-chip-${p}`} onClick={() => onChange({ ...filters, maxPrice: p })} className={`flex-1 py-1 rounded text-xs font-medium transition-colors ${filters.maxPrice === p ? 'bg-primary text-white' : 'bg-muted text-muted-foreground hover:bg-secondary'}`}>
                    ≤{p >= 1000 ? `${p / 1000}k` : p}
                  </button>
                ))}
              </div>
        ...
        <div className="p-4">
          ...
          {openSections.rating && (
            <div className="space-y-1.5">
              {RATINGS.map((r) => (
                <button key={`rating-${r}`} onClick={() => onChange({ ...filters, minRating: filters.minRating === r ? 0 : r })} className={`w-full flex items-center gap-2 px-3 py-2 rounded-lg text-sm transition-all ${filters.minRating === r ? 'bg-secondary border border-primary/30 text-primary' : 'hover:bg-muted text-foreground border border-transparent'}`}>
        ...
        <div className="p-4">
          ...
          {openSections.availability && (
            <label className="flex items-center gap-2.5 cursor-pointer">
              <div onClick={() => onChange({ ...filters, availableOnly: !filters.availableOnly })} className={`relative w-10 h-5 rounded-full transition-colors duration-200 cursor-pointer ${filters.availableOnly ? 'bg-primary' : 'bg-muted-foreground/30'}`}>
                <div className={`absolute top-0.5 w-4 h-4 rounded-full bg-white shadow transition-transform duration-200 ${filters.availableOnly ? 'translate-x-5' : 'translate-x-0.5'}`} />
              </div>
        ...
        <div className="p-4">
          <button onClick={() => toggleSection('distance')} className="flex items-center justify-between w-full mb-3">
          ...
          {openSections.distance && (
            <div className="flex flex-wrap gap-2">
              {DISTANCES.map((d) => (
                <button key={`dist-${d}`} onClick={() => onChange({ ...filters, maxDistance: d })} className={`px-3 py-1.5 rounded-lg text-xs font-semibold border transition-all ${filters.maxDistance === d ? 'gradient-green text-white border-transparent' : 'border-border text-muted-foreground hover:border-primary hover:text-primary'}`}>
                  {d} km
                </button>
              ))}
            </div>
